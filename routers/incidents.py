...
async def get_incidents(id: int | None = None, db: Session = Depends(get_db)):
...
@router.patch("/{id}")
async def patch_incidents(id: int, incidentPatch: IncidentPatch, db: Session = Depends(get_db)):
